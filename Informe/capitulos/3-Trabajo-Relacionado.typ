...
Estas relaciones son las que determinan por donde fluye el tráfico entre los ASes, 
ya que cada uno define sus políticas en base a sus necesidades y modelos de negocio. 
Existen dos tipos principales de relaciones entre los Sistemas Autónomos: Customer-Provider 
(C2P), donde un cliente paga a un provedor para accerder a partes del internet que no 
puede llegar y Peering (P2P), donde dos AS intercambian tráfico destinado a sus respectivos 
...
Customer-Provider se representan mediante una arista dirigida. Estos acuerdos son más comunes 
...
métodos heurísticos   //FIXME: Agregar referencia de los metodos heuristicos InferringASRelatioships2001 @computing-observed-autonomous-system-relationships-in-the-internet .
y basandose en los datos públicos sobre enrutamiento BGP, como las tablas de enrutamiento (RIBs) y los anuncios BGP.

Gao presentó un primer algoritmo basado en la idea de que el grado de un AS en el grafo 
suele ser mayor que el de sus clientes, mientras que los ASes peers tienden a tener un grado similar.
Su método también incorporó el concepto de _valley-free_ (VF) en los caminos de enrutamiento.
...
Luckie et al. @ASRank introdujeron el algoritmo _AS-Rank_ considerado el estado del arte para inferir relaciones C2P y P2P utilizando datos de BGP.
...
Así, introdujeron un nuevo algoritmo para inferir el "cono de clientes" de un AS, que es el conjunto de AS que puede alcanzar utilizando enlaces P2C.
 
Por último, Shapira y Shavitt @UnveilingtheTypeRelationshipBetweenAutonomousSystemsUsingDeepLearning emplearon técnicas de Deep Learning para la tarea de clasificación.
Utilizaron BGP2Vec con el objetivo de crear representaciones de los Sistemas Autónomos y luego pasaron estos _embeddings_ 
...
la inferencia de Sistemas Autónomos utilizando por primera vez técnicas de Deep Learning. Este método se basa en la creación de _embeddings_ de estos Sistemas Autónomos utilizando únicamente anunanuncios BGP provenientes de datasets públicos.
...
Una vez entrenado los _embeddings_ para cada ASN, se continúa con la tarea de clasificación de las relaciones entre los Sistemas Autónomos. Para esto se ocupa una Red Neuronal Artificial de dos capas.
...
ProbLink es un algoritmo probabilístico para inferir las relaciones entre Sistemas Autónomos propuesto por Yuchen Jin et al. @ProbLink. 
