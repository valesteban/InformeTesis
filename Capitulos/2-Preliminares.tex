...
Más información sobre los Sistemas Autónomos y los IXPs puede encontrarse en los Anexos \ref{sec:anexo_sistemas_autonomos} y \ref{sec:anexo_internet_exchange_points}, respectivamente.
...
A diferencia de otros protocolos de enrutamiento que buscan principalmente caminos mínimos o métricas de costo, BGP es un protocolo \textbf{basado en políticas}. Esto significa que la selección y propagación de rutas no depende únicamente de criterios técnicos, sino también de reglas administrativas y acuerdos comerciales definidos por cada Sistema Autónomo. Por esta razón, BGP no solo refleja conectividad entre redes, sino también decisiones económicas y estratégicas que influyen directamente en la estructura observable del enrutamiento interdominio.\\
...
Uno de los atributos más relevantes en BGP para esta tesis es \texttt{AS\_PATH}, el cual registra la secuencia de Sistemas Autónomos atravesados por un anuncio antes de alcanzar el AS que lo recibe. Este atributo permite reconstruir caminos observados entre ASes y, por lo tanto, construir una aproximación de la topología AS-level de Internet. En este trabajo, las rutas extraídas desde las tablas RIB se representan precisamente a partir de este atributo, ya que cada secuencia de ASes permite identificar conexiones adyacentes observadas en el plano de control.\\
...
% TODO:
\subsection{Prefijos IP y su asociación con Sistemas Autónomos}
Un \textbf{prefijo IP} corresponde a un bloque de direcciones IP representado mediante notación CIDR (\textit{Classless Inter-Domain Routing}), por ejemplo \texttt{192.0.2.0/24}. Esta notación indica qué parte de la dirección identifica a la red y qué parte queda disponible para los hosts. En Internet, los prefijos constituyen las unidades básicas de alcanzabilidad que se anuncian mediante BGP.\\

Cada Sistema Autónomo administra o anuncia uno o más prefijos IP, los cuales representan redes que pueden ser alcanzadas a través de dicho AS. Cuando un AS anuncia un prefijo en BGP, informa a sus vecinos que ese bloque de direcciones puede alcanzarse a través suyo, ya sea porque es el origen del prefijo o porque conoce una ruta válida hacia él.\\

A medida que el anuncio de un prefijo se propaga entre distintos Sistemas Autónomos, el atributo \texttt{AS\_PATH} registra la secuencia de ASes atravesados por la ruta. De esta manera, BGP no solo indica qué prefijos son alcanzables, sino también a través de qué secuencia de Sistemas Autónomos puede llegarse a ellos. En esta tesis, esta relación entre prefijos y caminos AS es fundamental, ya que las rutas observadas en las tablas RIB permiten construir una aproximación de la topología AS-level de Internet.\\

Así, las tablas RIB almacenan anuncios de prefijos junto con atributos como \texttt{AS\_PATH}, constituyendo una fuente clave para reconstruir rutas observadas entre Sistemas Autónomos.\\
...
En esta tesis, las tablas RIB son especialmente importantes porque constituyen la fuente principal desde la cual se extrajeron las rutas BGP utilizadas para construir el grafo de Internet. En particular, el atributo \texttt{AS\_PATH} contenido en estas tablas permite identificar pares adyacentes de Sistemas Autónomos y generar la estructura de conexiones observadas entre ellos. De esta manera, las RIBs actúan como una representación del conocimiento de alcanzabilidad mantenido por los routers BGP y, al mismo tiempo, como una fuente de datos clave para el análisis topológico realizado en este trabajo.\\
...
% TODO: Revisar/leer
\subsection{Embeddings y representaciones vectoriales}

Un \textit{embedding} corresponde a una representación vectorial densa de una entidad en un espacio de menor dimensión, diseñada para capturar propiedades relevantes de dicha entidad. En el contexto de aprendizaje automático, los embeddings permiten transformar objetos discretos o estructurados, como palabras, nodos o Sistemas Autónomos, en vectores numéricos que pueden ser utilizados como entrada para modelos predictivos.\\

En esta tesis, los embeddings de nodos representan a los Sistemas Autónomos dentro de la topología AS-level de Internet. La idea es que nodos con roles o posiciones estructurales similares dentro del grafo deberían poseer representaciones vectoriales cercanas en el espacio latente. Estas representaciones pueden luego utilizarse para tareas posteriores, como la clasificación de relaciones entre pares de ASes.\\

En el caso de las Redes Neuronales de Grafos, los embeddings se aprenden a partir de la estructura de conectividad del grafo y, cuando están disponibles, también a partir de atributos asociados a los nodos. De esta manera, la representación final de cada nodo combina información local, estructural y semántica, permitiendo modelar tareas más complejas que aquellas basadas únicamente en atributos tabulares.
...
Antes de describir la arquitectura \textit{Graph Attention Network} (GAT), es útil introducir el concepto de \textbf{mecanismo de atención}. En términos generales, un mecanismo de atención permite que un modelo asigne distinta importancia relativa a los elementos de entrada al momento de construir una representación. En lugar de tratar toda la información de manera uniforme, el modelo aprende qué partes son más relevantes para la tarea que se desea resolver.\\

En el caso de grafos, esto significa que un nodo no necesariamente debe considerar a todos sus vecinos con el mismo peso, sino que puede aprender cuáles vecinos aportan más información para actualizar su representación. En una GAT, este mecanismo se implementa aprendiendo coeficientes de atención para cada par de nodos conectados. Estos coeficientes indican cuánto influye cada vecino en la nueva representación del nodo central, permitiendo una agregación más flexible que la suma o el promedio uniforme utilizados en otras arquitecturas.
