...
El Algoritmo~\ref{alg:bgp_extraccion_filtrado} resume el procedimiento de descarga y filtrado inicial de rutas BGP. 
El pseudocódigo omite detalles de implementación particulares, como el uso de la biblioteca \texttt{BGPStream}, la conversión exacta de fechas a tiempo Unix y el manejo explícito de archivos, los cuales pertenecen al nivel de implementación y no alteran la lógica general del proceso. La implementación concreta puede consultarse en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:bgpstream_download}.

\begin{algorithm}[H]
\caption{Extracción y filtrado inicial de rutas BGP}
\label{alg:bgp_extraccion_filtrado}
\begin{algorithmic}[1]
\Require Fecha de inicio $f$, duración $d$
\Ensure Archivo con rutas BGP únicas y filtradas
\State Convertir $f$ a tiempo Unix
\State Inicializar flujo BGP en el intervalo $[f, f+d]$
\State Filtrar únicamente registros de tipo \texttt{ribs}
\State Inicializar conjunto de rutas únicas $P$
\For{cada registro del flujo}
    \For{cada elemento del registro}
        \State Extraer \texttt{AS\_PATH} y prefijo
        \If{\texttt{AS\_PATH} contiene estructuras ambiguas}
            \State Continuar con el siguiente elemento
        \EndIf
        \If{el prefijo corresponde a IPv4 y la ruta no pertenece a $P$}
            \State Guardar la ruta en archivo de salida
            \State Agregar la ruta a $P$
        \EndIf
    \EndFor
\EndFor
\State Cerrar archivo de salida
\end{algorithmic}
\end{algorithm}
...
% \begin{lstlisting}[language=Python, caption=Extracto de implementación para descarga y filtrado de rutas BGP con BGPStream, label=code:bgpstream_download]
% import datetime
% import pybgpstream
...
% TODO: Revisar/leer
En términos prácticos, este bloque realiza las siguientes operaciones sobre los datos reales:
\begin{enumerate}
    \item Convierte la fecha de entrada a tiempo Unix para definir la ventana temporal de consulta.
    \item Crea un flujo \texttt{BGPStream} y filtra únicamente registros de tipo \texttt{ribs}.
    \item Recorre registros y elementos del flujo, extrayendo los campos \texttt{as-path} y \texttt{prefix}.
    \item Descarta rutas con estructuras no simples en \texttt{AS\_PATH} (por ejemplo, presencia de \texttt{\{} o \texttt{(}), para evitar casos ambiguos en el grafo.
    \item Conserva solo rutas IPv4 (cuando el prefijo no contiene \texttt{:}) y elimina duplicados usando un conjunto en memoria.
    \item Escribe el resultado en \texttt{data/rib.txt}, reemplazando espacios por \texttt{|} para dejar un ASN por separador.
\end{enumerate}

El resultado de esta etapa es un archivo con rutas BGP únicas y en formato homogéneo, que luego se utiliza en la limpieza y construcción del grafo.

Cabe destacar que se optó por utilizar un sistema operativo basado en Linux, 
debido a la mayor compatibilidad y facilidad de uso e instalación de 
bibliotecas como \texttt{BGPStream} en comparación con otros sistemas 
operativos como Windows. Asimismo, se trabajó con un entorno virtual, 
con el fin de evitar conflictos entre versiones de las distintas 
bibliotecas utilizadas, considerando su continua actualización y la 
compatibilidad con scripts provenientes de otros repositorios utilizados 
en distintas partes del proyecto.\\


\subsection{Formato y Limpieza de las Rutas BGP}

Las rutas recolectadas se guardaron en un archivo de nombre \texttt{rib.txt}. Su formato consiste en una ruta por línea y los ASNs de un mismo path separados por el carácter ``\texttt{|}''. La decisión de utilizar este formato se tomó debido a su simplicidad y la compatibilidad con otros métodos utilizados en el \textit{benchmark} (Sección~\ref{sec:Benchmark_section}). A continuación, se muestra un ejemplo del formato utilizado en el Listado~\ref{lst:sanitized_ribs_format}.\\
...
% \end{lstlisting}
{
\renewcommand{\lstlistingname}{Formato}
\begin{lstlisting}[caption={Ejemplo del formato del archivo \texttt{sanitized\_ribs.txt}}, label={lst:sanitized_ribs_format}]
12350|6939|7843|11351|40625
61218|6939|59105|45679
50629|6461|52320|23106|53216
\end{lstlisting}
}
El formato~\ref{lst:sanitized_ribs_format} muestra un ejemplo de la forma utilizada para almacenar las rutas BGP procesadas. En este caso no se presenta pseudocódigo, ya que el objetivo no es describir un procedimiento, sino ilustrar la estructura concreta de los datos de entrada o salida utilizada por los experimentos.
...
El resultado de este proceso se guarda en un archivo de nombre \texttt{sanitized\_ribs.txt}, manteniendo el mismo formato descrito en el Listado~\ref{lst:sanitized_ribs_format}.\\
...
Como el dataset de CAIDA solo incluía relaciones P2C, se generaron relaciones C2P invirtiendo el orden de los ASes y ajustando sus etiquetas, en caso de existir esa relación en el grafo creado a partir de las RIBs. La codificación final de etiquetas utilizada en esta tesis se resume en la Tabla~\ref{tab:valores_relaciones}.\\

\begin{table}[H]
\centering
\caption{Codificación de etiquetas de relaciones entre ASes utilizada en esta tesis.}
\label{tab:valores_relaciones}
\begin{tabular}{lc}
\hline
\textbf{Tipo de relación} & \textbf{Etiqueta} \\
\hline
P2P (\textit{peer-to-peer}) & 0 \\
C2P (\textit{customer-to-provider}) & 1 \\
P2C (\textit{provider-to-customer}) & 2 \\
\hline
\end{tabular}
\end{table}
...
utils.py
\end{verbatim}
La estructura anterior resume la organización del repositorio utilizado en esta etapa. No se presenta como pseudocódigo, ya que no describe una secuencia algorítmica, sino la distribución de archivos y módulos que componen la implementación experimental.
...
{
\renewcommand{\lstlistingname}{Formato}
\begin{lstlisting}[caption=Salida de los resultados de métodos de inferencia, label=code:formato_out_benchmark]
<provider as>|<customer-as>|2|<real relationship>
<customer-as>|<provider as>|1|<real relationship> 
<peer-as>|<peer-as>|0|<real relationship> 
\end{lstlisting}
...
\begin{algorithm}[H]
\caption{Carga estandarizada de rutas BGP desde archivo}
\label{alg:carga_rutas_bgp}
\begin{algorithmic}[1]
\Require Archivo de rutas sanitizadas $A$, máximo de rutas $M$
\Ensure Lista de rutas BGP
\State Inicializar lista vacía $R$
\For{cada línea $l$ en $A$}
    \If{se alcanzó el límite $M$}
        \State Terminar lectura
    \EndIf
    \State Separar $l$ usando ``\texttt{|}''
    \State Agregar la secuencia resultante a $R$
\EndFor
\State Retornar $R$
\end{algorithmic}
\end{algorithm}
...
% TODO: Revisar/leer
El Algoritmo~\ref{alg:carga_rutas_bgp} presenta la estrategia utilizada para importar las rutas BGP desde el archivo \texttt{sanitized\_ribs.txt} y transformarlas a una representación homogénea en memoria. 
En la implementación concreta, esta estructura se almacena como una lista de listas, donde cada sublista representa un \texttt{AS\_PATH}, y además se incorpora un límite máximo de lectura para controlar el 
volumen de datos utilizado en cada experimento. Un extracto del código correspondiente puede verse en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:importar_bgp_routes}.
Con esto, se definió que todos los métodos del \textit{benchmark} recibirían esta variable como entrada estandarizada y por tanto del mismo archivo y con las mismas rutas.\\

Otro proceso de estandarización que se tuvo que integrar para el correcto funcionamiento del \textit{benchmark}, fue identificar y adaptar los formatos de salida generados por cada algoritmo. A continuación, se muestra  brevemente la estructura de salida de cada uno de los métodos utilizados:\\
...
\item \textbf{GAO y RUAN}:
    ...
    {
    \renewcommand{\lstlistingname}{Formato}
        \begin{lstlisting}[caption=Formato de salida del algoritmo GAO y RUAN., label=code:formato_salida_gao_ruan]
        <peer-as>|<peer-as>|0
...
\item \textbf{ProbLink y TopoScope}:
    {
    \renewcommand{\lstlistingname}{Formato}
    \begin{lstlisting}[caption=Formato de salida del algoritmo ProbLink y TopoScope. , label=code:formato_salida_problink]
    <provider-as>|<customer-as>|-1
    <peer-as>|<peer-as>|0
...
\item \textbf{BGP2Vec}:
    {
    \renewcommand{\lstlistingname}{Formato}
    \begin{lstlisting}[caption=Formato de salida de nuestro formato estandar definido. , label=code:formato_salida_estandar_mia]
    <peer-as>|<peer-as>|0
    <customer-as>|<provider-as>|1
...
Tras ejecutar los \textit{scripts} de cada método y obtener sus resultados de inferencia, se continuó con la estandarización de las salidas. Esta etapa consistió en convertir cada salida al formato previamente definido (Código~\ref{code:formato_salida_estandar_mia}). Un ejemplo concreto de esta transformación puede consultarse en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:transformacion_estandar_gao}.
...
% TODO: Revisar/leer reemplaza los dos coidigos
\begin{algorithm}[H]
\caption{Estandarización y limpieza de predicciones}
\label{alg:estandarizacion_limpieza_predicciones}
\begin{algorithmic}[1]
\Require Predicciones de un método $Y_{pred}$, pares evaluados $X$, etiquetas reales $Y$
\Ensure Predicciones y etiquetas en formato común
\State Inicializar listas vacías $X'$, $Y'$, $Y'_{pred}$
\For{cada predicción $p_i$ asociada al par $x_i$}
    \State Convertir $p_i$ al esquema estándar de etiquetas
    \If{$p_i$ corresponde a una relación válida}
        \State Agregar $x_i$ a $X'$
        \State Agregar su etiqueta real a $Y'$
        \State Agregar la predicción convertida a $Y'_{pred}$
    \EndIf
\EndFor
\State Retornar $X'$, $Y'$, $Y'_{pred}$
\end{algorithmic}
\end{algorithm}
El Algoritmo~\ref{alg:estandarizacion_limpieza_predicciones} resume la etapa de normalización de las salidas de los distintos métodos del benchmark. En la implementación concreta, 
el mapeo exacto depende del formato particular de salida de cada algoritmo, por lo que algunas 
etiquetas deben transformarse y otras descartarse. 
Posteriormente, también se eliminan aquellas inferencias cuya existencia no puede ser validada en el conjunto de evaluación, con el fin de asegurar una comparación homogénea entre métodos. Los extractos de implementación asociados a esta etapa se incluyen en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Códigos~\ref{anxcode:transformacion_estandar_gao} y~\ref{anxcode:limpieza_valores_no_encontrados}.


Junto a la transformación del formato, se verifica la existencia de cada relación inferida en el dataset de evaluación y se eliminan aquellas relaciones cuya existencia no pudo ser validada en el dataset de evaluación. Un extracto del código utilizado para esta limpieza se presenta en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:limpieza_valores_no_encontrados}.\\
...
% TODO: revisar/leer (se reemplaza este código por de los siguients hartos )
% Por lso que estan en parentesis de la forma (x)
%
\begin{algorithm}[H]
\caption{Integración de BGP2Vec y mapeo de ASNs a índices}
\label{alg:bgp2vec_integracion}
\begin{algorithmic}[1]
\Require Rutas BGP $R$, pares etiquetados de entrenamiento y prueba
\Ensure Embeddings y conjuntos mapeados a índices internos
\State Inicializar BGP2Vec con las rutas $R$ y los parámetros de entrenamiento
\State Entrenar el modelo para obtener embeddings de ASes
\State Construir diccionario $\texttt{asn\_to\_index}$
\For{cada par $(a,b)$ del conjunto de entrenamiento}
    \If{$a$ y $b$ existen en $\texttt{asn\_to\_index}$}
        \State Reemplazar $(a,b)$ por sus índices internos
        \State Guardar el par transformado y su etiqueta
    \EndIf
\EndFor
\For{cada par $(a,b)$ del conjunto de prueba}
    \If{$a$ y $b$ existen en $\texttt{asn\_to\_index}$}
        \State Reemplazar $(a,b)$ por sus índices internos
        \State Guardar el par transformado y su etiqueta
    \EndIf
\EndFor
\State Guardar embeddings y conjuntos transformados
\end{algorithmic}
\end{algorithm}

% TODO: revisar/leer 
El Algoritmo~\ref{alg:bgp2vec_integracion} resume la adaptación de BGP2Vec al flujo experimental de esta tesis. En la implementación real, fue necesario modificar la entrada del método para trabajar directamente con el archivo \texttt{sanitized\_ribs.txt}, además de crear un diccionario que asociara cada ASN con un índice interno, ya que el modelo opera sobre identificadores enteros y no directamente sobre los números de Sistema Autónomo. En esta etapa también se filtran aquellos pares para los cuales alguno de los ASNs no posee representación aprendida. Los fragmentos concretos de inicialización y mapeo pueden consultarse en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Códigos~\ref{anxcode:bgp2vec_init} y~\ref{anxcode:bgp2vec_mapping}.
...
% TODO: revisar/leer
\begin{algorithm}[H]
\caption{Construcción del grafo de Internet en formato DGL}
\label{alg:construccion_grafo_dgl}
\begin{algorithmic}[1]
\Require Archivo de rutas sanitizadas, archivo de atributos de nodos
\Ensure Archivos \texttt{edges.csv}, \texttt{nodes.csv}, \texttt{graphs.csv} y \texttt{meta.yaml}
\State Inicializar grafo vacío $G$
\For{cada ruta del archivo de entrada}
    \State Obtener la secuencia de ASes del \texttt{AS\_PATH}
    \For{cada par adyacente de ASes}
        \State Agregar arista dirigida al grafo
        \State Registrar los nodos involucrados
    \EndFor
\EndFor
\State Incorporar atributos a los nodos desde el archivo correspondiente
\If{se desea simplificar la topología}
    \State Eliminar nodos según el criterio de grado definido
\EndIf
\State Exportar la estructura y atributos en formato compatible con DGL
\end{algorithmic}
\end{algorithm}
El Algoritmo~\ref{alg:construccion_grafo_dgl} resume la creación del dataset del grafo de Internet a partir de las rutas BGP sanitizadas. En el código real, esta lógica fue encapsulada en una clase que permite crear la topología base, incorporar atributos a los nodos y aplicar operaciones de simplificación, como la eliminación iterativa de nodos hoja. Finalmente, el resultado se exporta en los archivos requeridos por el formato de datasets de DGL. Un extracto de esta implementación se incluye en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:graph_class}.
...
El Algoritmo~\ref{alg:gnn_encoder_decoder} resume una clase representativa de un modelo basado en GCN. En este ejemplo, el método \textit{encode} realiza el \textit{forward pass} de las capas GNN, mientras que los métodos \textit{decodeDotProduct}, \textit{decodeMLP} y \textit{decodeBilinear} actúan como \textit{decoders}.\\
...
% TODO: revisar/leer
\begin{algorithm}[H]
\caption{Esquema general de un modelo GNN encoder--decoder}
\label{alg:gnn_encoder_decoder}
\begin{algorithmic}[1]
\Require Grafo $G$, atributos de nodos $X$
\Ensure Embeddings de nodos o predicciones sobre nodos/aristas
\State Agregar auto-conexiones al grafo
\State Aplicar una primera capa GNN sobre $(G, X)$
\State Aplicar función de activación y \textit{dropout}
\State Aplicar una segunda capa GNN para obtener embeddings $H$
\If{la tarea usa decodificador por producto punto}
    \State Calcular puntajes sobre aristas a partir de $H$
\ElsIf{la tarea usa decodificador MLP}
    \State Predecir usando una MLP sobre pares de embeddings
\ElsIf{la tarea usa decodificador bilineal}
    \State Predecir usando una transformación bilineal
\Else
    \State Aplicar una capa lineal para predicción nodal
\EndIf
\State Retornar embeddings o predicciones
\end{algorithmic}
\end{algorithm}
El Algoritmo~\ref{alg:gnn_encoder_decoder} sintetiza la estructura general de los modelos GNN utilizados en esta tesis bajo un esquema encoder--decoder. En la implementación concreta se definieron capas específicas según la arquitectura elegida (por ejemplo, GCN, GraphSAGE o GAT), además de distintos decodificadores para adaptarse a tareas de predicción de enlaces, clasificación de relaciones o predicción de atributos nodales. El código también incorpora operaciones propias de DGL para aplicar funciones sobre las aristas y manipular los atributos internos del grafo. Un ejemplo representativo puede consultarse en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:modelo_gcn}.
...
Dado que los modelos de embeddings trabajan con IDs en lugar de valores simbólicos como los ASN (forma en que nuestro dataset ToR era identificado), se tuvo que adaptar el conjunto de entrenamiento y prueba. Específicamente, se mapeó cada ASN a un ID utilizando un diccionario que se creó previamente en el trabajo; si alguno de los ASN no era encontrado en dicho diccionario, el nodo era descartado del conjunto. El código de esta etapa se incluye en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:mapeo_asn_id}.\\
...
% TODO: revisar/leer
\begin{algorithm}[H]
\caption{Clasificación de relaciones entre ASes usando una CNN sobre embeddings}
\label{alg:cnn_relaciones_as_resumen}
\begin{algorithmic}[1]
\Require Pares de ASes representados por índices, matriz de embeddings
\Ensure Predicción de la relación entre pares de ASes
\State Obtener embeddings asociados a cada par de entrada
\State Aplicar una primera capa convolucional
\State Aplicar una operación de \textit{pooling}
\State Aplicar una segunda capa convolucional
\State Aplicar una nueva operación de \textit{pooling}
\State Aplanar la representación resultante
\State Aplicar una capa densa intermedia
\State Aplicar una capa final con \textit{softmax}
\State Retornar la clase predicha
\end{algorithmic}
\end{algorithm}
El Algoritmo~\ref{alg:cnn_relaciones_as_resumen} resume la arquitectura empleada para clasificar relaciones entre ASes a partir de embeddings previamente generados. En el código real se especifican detalles adicionales, como la dimensión del vocabulario, el tamaño de los embeddings, la posibilidad de fijar o entrenar la capa \texttt{Embedding}, el número de filtros convolucionales y los reacomodos de dimensión necesarios antes de cada etapa de \textit{pooling}.
...
% \begin{algorithm}[H]
% \caption{Clasificación de relaciones entre ASes usando una CNN sobre embeddings}
% \label{alg:cnn_relaciones_as}
% \begin{algorithmic}[1]
% \Require Pares de ASes representados por índices, matriz de embeddings
% \Ensure Predicción de la relación entre pares de ASes
% \State Obtener embeddings asociados a cada par de entrada
% \State Aplicar una primera capa convolucional
% \State Aplicar operación de \textit{pooling}
% \State Aplicar una segunda capa convolucional
% \State Aplicar una nueva operación de \textit{pooling}
% \State Aplanar la representación resultante
% \State Aplicar una capa densa intermedia
...
% \end{algorithmic}
% \end{algorithm}
El Algoritmo~\ref{alg:cnn_relaciones_as_resumen} sintetiza la arquitectura empleada para clasificar relaciones entre ASes a partir de embeddings previamente generados. En el código real se especifican hiperparámetros concretos, como el tamaño de los filtros, el número de canales, las operaciones de reacomodo de dimensiones y la posibilidad de mantener fija o entrenable la capa de embeddings. La implementación correspondiente se presenta en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:cnn_bgp2vec_model}.
...
% TODO: revisar/leer
\begin{algorithm}[H]
\caption{Cálculo de pesos por clase para balanceo}
\label{alg:pesos_clase}
\begin{algorithmic}[1]
\Require Etiquetas del conjunto de entrenamiento $Y$
\Ensure Diccionario de pesos por clase
\State Contar la frecuencia de cada clase en $Y$
\For{cada clase $c$}
    \State Calcular un peso inversamente proporcional a su frecuencia
\EndFor
\State Construir diccionario \texttt{clase} $\rightarrow$ \texttt{peso}
\State Retornar diccionario de pesos
\end{algorithmic}
\end{algorithm}

El Algoritmo~\ref{alg:pesos_clase} resume el procedimiento utilizado para mitigar el desbalance de clases presente en la tarea de inferencia. En la implementación concreta, estos pesos se calculan automáticamente a partir de la distribución observada en el conjunto de entrenamiento y luego se incorporan a la función de pérdida, asignando mayor importancia a las clases menos representadas. Un extracto del código usado para este cálculo se muestra en el Anexo~\ref{sec:anexo_fragmentos_codigo}, Código~\ref{anxcode:class_weights_dict}.
