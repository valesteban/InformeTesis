...
\textbf{Machine Learning} para automatizar este 
...
Las Redes Neuronales de Grafos (GNNs) son capaces de capturar patrones estructurales relevantes 
presentes en la topología AS-level de Internet, representada a partir de información de enrutamiento BGP,
 y que dichas representaciones pueden ser utilizadas para inferir relaciones económicas entre Sistemas Autónomos, 
obteniendo un desempeño comparable en Accuracy a métodos existentes del estado del arte.~\cite{UnveilingtheTypeRelationshipBetweenAutonomousSystemsUsingDeepLearning,ASRank}
...
El objetivo de esta tesis es analizar el comportamiento de distintas arquitecturas de 
\textbf{Redes Neuronales de Grafos (GNNs)} sobre la topología de Internet a nivel de Sistemas Autónomos, 
representada como un grafo construido a partir de información de 
enrutamiento BGP.\\

En particular, se estudia si estos modelos son capaces de capturar patrones estructurales del grafo AS-level 
y contribuir a la inferencia de relaciones económicas entre Sistemas Autónomos. Para ello, se desarrolla un 
\textbf{benchmark experimental} que permite comparar distintas arquitecturas de GNN con métodos heurísticos 
y de Machine Learning del estado del arte.
